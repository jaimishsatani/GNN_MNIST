...
class GraphNet(nn.Module):
    def __init__(self, image_size=28, pred_edge=False):
        ...
        if pred_edge:
            ...
            coord_normalized = (coord - np.mean(coord, axis=0)) / (np.std(coord, axis=0) + 1e-5)  # Normalize
            ...
            self.pred_edge_fc = nn.Sequential(nn.Linear(4, 64),
                                              ...
                                              nn.Linear(64, 1),
                                              nn.Tanh())
            self.register_buffer('adjacency_matrix', adjacency_matrix)  # not to be considered a model parameter
        else:
            # Use a pre-computed adjacency matrix
            A = self.precompute_adjacency_images(image_size)
            self.register_buffer('A', A)  # not to be considered a model parameter
    ...
    def forward(self, x):
        ...
        if self.pred_edge:
            self.A = self.pred_edge_fc(self.adjacency_matrix).squeeze() 
    ...
    def precompute_adjacency_images(image_size):
        """
        Precompute adjacency matrix for MNIST images.
        """
        print('precompute_adjacency_images')
        col, row = np.meshgrid(np.arange(image_size), np.arange(image_size))  # (28 x 28)
        coord = np.stack((col, row), axis=2).reshape(-1, 2) / image_size 
        ...
        # Some additional trick I found useful
...
def main():
    ...
    parser.add_argument('--pred_edge', action='store_true', default=False, help='predict edges instead of using predefined ones')
